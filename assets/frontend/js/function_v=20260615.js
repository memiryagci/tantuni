(function ($) {
	...
	/* Skill Bar */
	if ($('.skills-progress-bar').length && $.fn.waypoint) {
		try { $('.skills-progress-bar').waypoint(function() {
			$('.skillbar').each(function() { $(this).find('.count-bar').animate({ width:$(this).attr('data-percent') },2000); });
		},{ offset: '50%' }); } catch(e) {}
	}

	/* Youtube Background Video JS */
	if ($('#herovideo').length && $.fn.YTPlayer) {
		try { var myPlayer = $("#herovideo").YTPlayer(); } catch(e) {}
	}

	/* Init Counter */
	if ($('.counter').length && $.fn.counterUp) {
		try { $('.counter').counterUp({ delay: 6, time: 3000 }); } catch(e) {}
	}
	...
	/* text-anime-style-1 - SplitText removed, using simple fade */
	if ($('.text-anime-style-1').length && typeof SplitText !== 'undefined') {
		try {
		let staggerAmount = 0.05, translateXValue = 0, delayValue = 0.5;
	...
	/* text-anime-style-2 - SplitText guard */
	...
	/* text-anime-style-3 - SplitText guard */
	...
	if ($.fn.magnificPopup) { $('.gallery-items').magnificPopup({
		delegate: 'a',
		type: 'image',
	...
	if ($.fn.validator) { $contactform.validator({focus: false}); }
	...
	function submitForm(){
		...
		$.ajax({
			...
			success : function(text){
				$contactform[0].reset();
				if(form_type_id==1){
					alert("Franchise başvuru talebiniz başarıyla gönderildi. En kısa sürede tarafınıza dönüş yapacağız.");
					submitMSG(true, "Franchise başvuru talebiniz başarıyla gönderildi. En kısa sürede tarafınıza dönüş yapacağız.")
				}
				else {
					alert("İletişim bilgileriniz başarıyla gönderildi. En kısa sürede tarafınıza dönüş yapacağız.");
					submitMSG(true, "İletişim bilgileriniz başarıyla gönderildi. En kısa sürede tarafınıza dönüş yapacağız.")
				}
	...
	if ($.fn.validator) { $appointmentForm.validator({focus: false}); }
